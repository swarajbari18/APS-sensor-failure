...
```bash
git clone https://github.com/swarajbari18/APS-sensor-failure.git
```
...
###### No Conda? No Problem
You can use a venv environment instead, 
If you are in ubuntu 20.04 then it by default has python 3.8 installed on it
You can either install a new python version python 3.7.6 or just use python 3.8
But I reccommend to install python 3.7.6 or any other for that matter
because the inbuilt python in ubuntu is used by the system, so we would not like to mess it up,
but if you don't wish to then you may go ahead with default python because I checked the same code inside a Docker container with python 3.8 base Image so it should run fine
